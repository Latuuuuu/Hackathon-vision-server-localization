...
public:
    HomographyArucoNode() : Node("homography_all_node") {
        ...
        this->declare_parameter<std::vector<double>>("pantry_area", std::vector<double>{});
        ...
        robot_height_ = this->get_parameter("robot.height").as_double();
        ...
        const auto pantry_area_param = this->get_parameter("pantry_area").as_double_array();
        ...
        robot_pose_pub_ = this->create_publisher<geometry_msgs::msg::PoseStamped>("/cb_camera_pose", 10);
        rival_pose_pub_ = this->create_publisher<geometry_msgs::msg::PoseStamped>("/cb_rival_pose", 10);
        ...
        if (pantry_area_param.size() % 4 != 0 || pantry_area_param.empty()) {
            RCLCPP_WARN(
                this->get_logger(),
                "pantry_area parameter size should be a multiple of 4, got %zu",
                pantry_area_param.size());
        } else {
            for (size_t i = 0; i < pantry_area_param.size(); i += 4) {
                Rect area;
                area.min_x = pantry_area_param[i];
                area.max_x = pantry_area_param[i + 1];
                area.min_y = pantry_area_param[i + 2];
                area.max_y = pantry_area_param[i + 3];

                pantry_areas_.push_back(area);
            }

            RCLCPP_INFO(
                this->get_logger(),
                "Loaded %zu pantry areas",
                pantry_areas_.size());
        }

        sima_score_pub_ = this->create_publisher<std_msgs::msg::Int32>("/sima_pantry_score", 10);
        pantry_sima_count = 0;
...
private:
    ...
    bool is_sima_in_pantry_area(const double x, const double y, const double yaw_rad) const {
        constexpr double sima_front_x = 0.054;
        constexpr double sima_back_x = -0.0761;
        constexpr double sima_half_y = 0.045;

        const double c = std::cos(yaw_rad);
        const double s = std::sin(yaw_rad);

        std::array<cv::Point2d, 4> sima_corners = {{
            {x + c * sima_front_x - s * sima_half_y,
            y + s * sima_front_x + c * sima_half_y},
            {x + c * sima_front_x - s * -sima_half_y,
            y + s * sima_front_x + c * -sima_half_y},
            {x + c * sima_back_x - s * -sima_half_y,
            y + s * sima_back_x + c * -sima_half_y},
            {x + c * sima_back_x - s * sima_half_y,
            y + s * sima_back_x + c * sima_half_y},
        }};

        auto project_points = [](const std::array<cv::Point2d, 4> &points,
                                const cv::Point2d &axis,
                                double &min_proj,
                                double &max_proj) {
            min_proj = points[0].x * axis.x + points[0].y * axis.y;
            max_proj = min_proj;

            for (size_t i = 1; i < points.size(); ++i) {
                const double proj = points[i].x * axis.x + points[i].y * axis.y;
                min_proj = std::min(min_proj, proj);
                max_proj = std::max(max_proj, proj);
            }
        };

        auto overlaps_on_axis = [&](const std::array<cv::Point2d, 4> &a,
                                    const std::array<cv::Point2d, 4> &b,
                                    const cv::Point2d &axis) {
            double min_a, max_a, min_b, max_b;
            project_points(a, axis, min_a, max_a);
            project_points(b, axis, min_b, max_b);
            return !(max_a < min_b || max_b < min_a);
        };

        auto rectangles_overlap = [&](const std::array<cv::Point2d, 4> &a,
                                    const std::array<cv::Point2d, 4> &b) {
            std::array<cv::Point2d, 4> axes = {{
            ...
            for (const auto &axis_raw : axes) {
                ...
                const cv::Point2d axis{axis_raw.x / norm, axis_raw.y / norm};
        ...
        for (const auto &area : pantry_areas_) {
            const std::array<cv::Point2d, 4> pantry_corners = {{
            ...
            if (rectangles_overlap(sima_corners, pantry_corners)) {
                return true;
            }
        }
    ...
    void RGB_img_callback(const sensor_msgs::msg::Image::SharedPtr msg) {
        try {
            ...
            bool is_rival_found = false;
            double rival_u = 0.0;
            double rival_v = 0.0;
            std::vector<cv::Point2f> rival_corners(4);
            ...
            if (!field_marker_ids.empty() || !sima_marker_ids.empty()) {
                ...
                for (size_t i = 0; i < field_marker_ids.size(); i++) {
                    ...
                    if (marker_world_2d_.count(id)) {
                        ...
                        target_u = (field_marker_corners[i][0].x + field_marker_corners[i][2].x) / 2.0;
                        target_v = (field_marker_corners[i][0].y + field_marker_corners[i][2].y) / 2.0;

                        for (int j=0;j<4;j++){
                            target_corners[j] = field_marker_corners[i][j];
                        }

                        is_target_found = true;
                    }
                    else if(id == rival_id_){
                        rival_u = (field_marker_corners[i][0].x + field_marker_corners[i][2].x) / 2.0;
                        rival_v = (field_marker_corners[i][0].y + field_marker_corners[i][2].y) / 2.0;

                        for (int j=0;j<4;j++){
                            rival_corners[j] = field_marker_corners[i][j];
                        }
                ...
                if (!any_sima_found && !is_target_found) {
                    sima_score_pub_->publish(std_msgs::msg::Int32().set__data(0));
                    return;
                }
                ...
                if (pts_img_2d.size() >= 4) {
                    ...
                    if (!H.empty()) {
                        ...
                        double raw_pose_rival[3] = {0.0, 0.0, 0.0};
                        double final_pose_rival[3] = {0.0, 0.0, 0.0};
                        double yaw_rad_rival = 0.0;
                        double yaw_deg_rival = 0.0;

                        if(is_target_found and (mode_ == "robot" || mode_ == "all")) {
                            // ================= robot =======================
                            ...
                            double t = (cam_z_ - robot_height_) / cam_z_;
                            double target_x = cam_x_ + t * (X_g - cam_x_);
                            double target_y = cam_y_ + t * (Y_g - cam_y_);

                            raw_pose[0] = target_x;
                            raw_pose[1] = target_y;
                            raw_pose[2] = robot_height_;
                            pose_filter_for_id(robot_id_, raw_pose, final_pose);
                            ...
                            // ============= rival (optional) =================
                            cv::Mat pt_rival_src = (cv::Mat_<double>(3, 1) << rival_u, rival_v, 1.0);
                            cv::Mat pt_rival_dst = H * pt_rival_src;

                            double X_g_rival = pt_rival_dst.at<double>(0, 0) / pt_rival_dst.at<double>(2, 0);
                            double Y_g_rival = pt_rival_dst.at<double>(1, 0) / pt_rival_dst.at<double>(2, 0);

                            double t_rival = (cam_z_ - robot_height_) / cam_z_;
                            double rival_x = cam_x_ + t_rival * (X_g_rival - cam_x_);
                            double rival_y = cam_y_ + t_rival * (Y_g_rival - cam_y_);

                            raw_pose_rival[0] = rival_x;
                            raw_pose_rival[1] = rival_y;
                            raw_pose_rival[2] = robot_height_;
                            pose_filter_for_id(rival_id_, raw_pose_rival, final_pose_rival);

                            cv::Point2f rival_left_center = (rival_corners[0] + rival_corners[3]) / 2.0;
                            cv::Point2f rival_right_center = (rival_corners[1] + rival_corners[2]) / 2.0;
                            cv::Mat pt_rival_left_src = (cv::Mat_<double>(3, 1) << rival_left_center.x, rival_left_center.y, 1.0);
                            cv::Mat pt_rival_left_dst = H * pt_rival_left_src;
                            double X_rival_left = pt_rival_left_dst.at<double>(0, 0) / pt_rival_left_dst.at<double>(2, 0);
                            double Y_rival_left = pt_rival_left_dst.at<double>(1, 0) / pt_rival_left_dst.at<double>(2, 0);
                            cv::Mat pt_rival_right_src = (cv::Mat_<double>(3, 1) << rival_right_center.x, rival_right_center.y, 1.0);
                            cv::Mat pt_rival_right_dst = H * pt_rival_right_src;
                            double X_rival_right = pt_rival_right_dst.at<double>(0, 0) / pt_rival_right_dst.at<double>(2, 0);
                            double Y_rival_right = pt_rival_right_dst.at<double>(1, 0) / pt_rival_right_dst.at<double>(2, 0);
                            double dx_rival = X_rival_right - X_rival_left;
                            double dy_rival = Y_rival_right - Y_rival_left;
                            yaw_rad_rival = std::atan2(dy_rival, dx_rival);
                            yaw_deg_rival = yaw_rad_rival * 180.0 / CV_PI;

                            geometry_msgs::msg::PoseStamped rival_pose_msg;
                            rival_pose_msg.header.stamp = msg->header.stamp;
                            rival_pose_msg.header.frame_id = world_frame_;
                            rival_pose_msg.pose.position.x = final_pose_rival[0];
                            rival_pose_msg.pose.position.y = final_pose_rival[1];
                            rival_pose_msg.pose.position.z = final_pose_rival[2];

                            const double half_yaw_rival = yaw_rad_rival * 0.5;
                            rival_pose_msg.pose.orientation.x = 0.0;
                            rival_pose_msg.pose.orientation.y = 0.0;
                            rival_pose_msg.pose.orientation.z = std::sin(half_yaw_rival);
                            rival_pose_msg.pose.orientation.w = std::cos(half_yaw_rival);

                            rival_pose_pub_->publish(rival_pose_msg);
                            // ============================================================
                        ...
                        if ((mode_ == "sima" || mode_ == "all")) {
                            ...
                            for (const int sima_id : sima_ids_) {
                                ...
                                bool in_pantry = is_sima_in_pantry_area(final_pose[0], final_pose[1], yaw_rad);
                                if (in_pantry) {
                                    pantry_sima_count++;
                                }
                                ...
                                if (is_debug_mode_) {
                                    RCLCPP_INFO(
                                        this->get_logger(),
                                        "sima_id=%d raw:(%.3f, %.3f, %.3f) filtered:(%.3f, %.3f, %.3f) yaw: %.3f rad %.3f deg inpantry: %s topic:/sima_%d",
                                        sima_id,
                                        raw_pose[0], raw_pose[1], raw_pose[2],
                                        final_pose[0], final_pose[1], final_pose[2],
                                        yaw_rad, yaw_deg,
                ...
                if (is_debug_mode_) {
                    if (image_debug_){
                        ...
                        cv::putText(RGB_frame, "Score: " + std::to_string(pantry_sima_count) + "*5 = " + std::to_string(pantry_sima_count * 5), cv::Point(10, 30), cv::FONT_HERSHEY_SIMPLEX, 1.0, cv::Scalar(0, 255, 0), 2);
            ...
            sima_score_pub_->publish(std_msgs::msg::Int32().set__data(pantry_sima_count * 5));
        } catch (cv_bridge::Exception& e) {
            RCLCPP_ERROR(this->get_logger(), "cv_bridge exception: %s", e.what());
        }
    }
    ...
    rclcpp::Publisher<geometry_msgs::msg::PoseStamped>::SharedPtr rival_pose_pub_;
    rclcpp::Publisher<std_msgs::msg::Int32>::SharedPtr sima_score_pub_;
    ...
    std::vector<Rect> pantry_areas_;
    int pantry_sima_count = 0;
